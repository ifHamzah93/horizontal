				...
				<form id="myForm" name="myForm" class="form-horizontal">
					...
					<div class="form-group">
						<div class="col-12">
							...
							<input type="text" class="form-control" name="nama" autocomplete="off" autocorrect="off" spellcheck="false" required="">
						</div>
					</div>
				</form>
			</div>
...
$(function () {
	    ...
	    buttons: [
            {
                ...
                action: function ( e, dt, node, config ) {
					...
					$('.modal-title').html("Create New Kategori");
					$('#myModal').modal('show');
                }
            }
        ],
	...
	$('body').on('click', '.edit', function () {
		var id = $(this).data('id');
		$.get("{{ route('master-kategori.index') }}" +'/' + id +'/edit', function (data) {
	...
	$('body').on('click', '.delete', function () {
		...
		if (confirm("Are You sure want to delete !")) {
			$.ajax({
				type: "DELETE",
				url: "{{ route('master-kategori.store') }}"+'/'+id,
				success: function (data) {
					$.notify(data.message,data.notify);
					table.ajax.reload( null, false);
				},
